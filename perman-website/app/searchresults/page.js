...
const SearchResults = () => {
    ...
    const [searchedEpisodes, setSearchedEpisodes] = useState(null)

    useEffect(() => {
        const fetchSearchedEpisodes = async () => {
            try {
                const accessToken = localStorage.getItem("accessToken")
                const response = await axiosInstance.get(`/search?episode=${searchParams.get("episode")}`, {
                    headers: {
  ...
  return (
    <>
        ...
        <div className="main mx-3 md:mx-10 my-20">
            <div className='flex flex-row justify-center items-center mb-20 relative'>
                ...
                <div className='flex flex-col gap-2 text-center w-3/4'>
                    <span className="text-2xl md:text-5xl font-bold">Search Results For "{searchParams.get("episode")}"</span>
                </div>
            </div>

            <span className='font-bold text-2xl'>Episodes: {searchedEpisodes ? searchedEpisodes.length : 0}</span>

            <ul className="mt-10 flex flex-col gap-10 max-h-[950px] overflow-y-auto">

                {searchedEpisodes && searchedEpisodes.map((episode) => (<li key={episode.episodeId} className='bg-blue-5-opacity w-[99%] h-72 lg:h-60 p-5 rounded-3xl flex flex-col gap-5 hover:bg-blue-10-opacity cursor-pointer transition-colors duration-300'>
                ...
                {searchedEpisodes && searchedEpisodes.length === 0 && <li className='h-96 flex justify-center items-center'>
                  <span className='font-bold text-3xl md:text-5xl'>No Episodes !</span>  
                </li>}

                {!searchedEpisodes && <li className='h-80 flex justify-center items-center'><Spinner/></li>}
